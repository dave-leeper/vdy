...
module.exports = (entry) => {
    return async (req, res, next) => {
        ...
        if (handlerArgs.preprocessor) {
            const {preprocessor} = require(handlerArgs.preprocessor)
        ...
        if (handlerArgs.validator) {
            const {validator} = require(handlerArgs.validator)
        ...
        if (handlerArgs.postprocessor) {
            const {postprocessor} = require(handlerArgs.postprocessor)
